<template>
  <v-container>
    <v-card class="pb-2">
      ...
      <v-spacer></v-spacer>
      <v-row class="pt-5 px-4" align="center">
        <v-col cols="12" md="4" lg="3">
          <v-select label="Tipo de talla" v-model="sizeType" item-text="name" :items="sizeTypes"
            prepend-icon="mdi-human-male-boy" return-object dense hide-details @change="fetchSizes"></v-select>
        </v-col>
        <v-col cols="12" md="4" lg="3">
          <v-select label="Estándar de talla" v-model="sizeStandard" item-text="name" item-value="value"
            :items="sizeStandards" prepend-icon="mdi-human-male-height-variant" :return-object="false" dense
            hide-details @change="fetchSizes"></v-select>
        </v-col>
                ...
                <td class="text-center">
                  <v-tooltip bottom>
                    <template v-slot:activator="{ on, attrs }">
                      <v-btn icon v-bind="attrs" v-on="on" color="error" @click="$refs.dialogRemove.showDialog(size)">
                        <v-icon dense>
                          mdi-close-circle
                        </v-icon>
                      </v-btn>
    ...
    <dialog-remove ref="dialogRemove" :female="true" type="talla" url="size" @updateList="fetchSizes" />
  </v-container>
</template>
