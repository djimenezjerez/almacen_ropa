  ...
  <v-container>
    <v-card>
      <v-toolbar color="secondary">
        ...
        <router-link style="text-decoration: none;" class="white--text text-h6 font-weight-light"
          :to="{ path: `/movements/TRANSFER` }">{{ movementType.name }}</router-link>
      </v-toolbar>
      ...
      <v-row class="px-4" align="center" justify="end">
        ...
        <v-col cols="12" md="6" xl="3" :class="{
          'text-right': $vuetify.breakpoint.smAndUp,
        }">
          <v-select :label="store.warehouse == undefined ? 'Tienda/Almacén' : (store.warehouse ? 'Almacén' : 'Tienda')"
            v-model="store" item-text="name" :items="stores" prepend-icon="mdi-store" return-object dense hide-details
            @change="products = []"></v-select>
        </v-col>
        <v-col cols="12" md="6" xl="3" :class="{
          'text-right': $vuetify.breakpoint.smAndUp,
        }">
          <add-button text="Seleccionar productos" block :disabled="Object.keys(store).length === 0"
