    ...
    <v-card>
      ...
      <div class="px-5 pb-5">
        <v-card-text>
          <v-row dense>
            <v-col cols="12">
              <v-select :items="sizeTypes" item-text="name" v-model="sizeType" label="Tipo de talla"
                prepend-icon="mdi-human-male-boy" return-object hide-details @change="fetchProductNames"></v-select>
            </v-col>
            <v-col cols="12">
              <v-autocomplete :items="productNames" item-text="product_name" v-model="productName" label="Nombre"
                prepend-icon="mdi-hanger" return-object persistent-hint :hint="productName.category_name"
                :disabled="Object.keys(sizeType).length === 0" @change="fetchGenders"></v-autocomplete>
            </v-col>
            <v-col cols="12">
              <v-select :items="genders" item-text="name" v-model="gender" label="Género"
                prepend-icon="mdi-gender-male-female" return-object hide-details
                :disabled="Object.keys(productName).length === 0" @change="fetchBrands"></v-select>
            </v-col>
            <v-col cols="12">
              <v-autocomplete :items="brands" item-text="name" v-model="brand" label="Marca"
                prepend-icon="mdi-shopping-outline" return-object hide-details
                :disabled="Object.keys(gender).length === 0" @change="fetchColors"></v-autocomplete>
            </v-col>
            <v-col cols="12">
              <v-autocomplete :items="colors" item-text="name" v-model="color" label="Color"
                prepend-icon="mdi-invert-colors" return-object hide-details :disabled="Object.keys(brand).length === 0"
                @change="fetchSizes"></v-autocomplete>
            </v-col>
            <v-col cols="12">
              <v-row align="center" align-content="space-between" dense>
                <v-col cols="11">
                  <v-select :items="sizes" item-text="size_name" return-object label="Tallas" multiple
                    v-model="products" prepend-icon="mdi-tshirt-crew-outline" chips deletable-chips hide-selected
                    :disabled="Object.keys(color).length === 0"></v-select>
                </v-col>
                <v-col cols="1">
                  <v-tooltip bottom>
                    <template v-slot:activator="{ on, attrs }">
                      <v-btn x-small fab color="success" v-bind="attrs" v-on="on"
                        :disabled="Object.keys(color).length === 0" @click="products = sizes">
        ...
        <v-card-actions>
          <v-row dense justify="end">
            <v-col cols="12" md="6">
              <v-btn block color="info" :disabled="products.length == 0" @click.stop="selectProducts">
                Aceptar
              </v-btn>
            </v-col>
          </v-row>
